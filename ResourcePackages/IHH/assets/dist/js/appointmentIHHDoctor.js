function isFormValid() {
  ...
  if ($("#txtCountryCode2").val().length > 0) {
    ...
    console.log(
      ...
      " " +
      countryCodeValidity($("#txtCountryCode2"))
    );
  }
...
$(document).ready(function () {
  ...
  function generateDoctorNameInDDL(doc) {
    ...
    for (let index = 0; index < doc.value.length; index++) {
      $("#ddlDoctors").append(
        '<option value="' + doc.value[index].Salutation + " " + doc.value[index].Name +
        '" class="doctor-appt">' +
  ...
  $("#attachefile").on("change", function () {
    ...
    if (filetype != "image/jpeg" && filetype != "application/pdf") {
      ...
      $(this).next().next(".select-error").html("Upload wrong file type");
      ...
      $(this).next().next(".select-error").css("display", "block");
      $(this).next().next(".select-error").html("File Size More Than 10MB");
      $("#attachefile").val(""); //clearing if invalid
      $('.custom-upload').addClass('error');
      if ($('.custom-upload').hasClass('uploaded')) {
        $('.custom-upload').removeClass('uploaded');
  ...
  function isAcknowledge() {
    ...
    if (!isAcknowledge) {
      ...
      $("#accurate").parent().next(".select-error").css("display", "none");
    }
  ...
  async function getData() {
    ...
    var parms = JSON.stringify({
      ...
      AppointmentType: $(".appointment-form").data("appointment-type"),
          ...
          getToken().then(async function () {
            $.ajax({
              ...
              success: function (data) {
                ...
                {
                  isHS = "?HealthScreening";
                }
                window.location.href = $("#ddlHospital").find(":selected").data("appturl") + "/thank-you" + isHS;
              },
              error: function (err) {
  ...
  $(".appointment-summary .title-back-button-wrapper").on('click', function () {
